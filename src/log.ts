 ...
 *
 * It exists because of where the errors go otherwise. A format that stopped
 * parsing, a writer that throws on one table shape — every one of them is
 * caught in `tools.ts` and turned into a sentence for the *model*, which reacts
 * to it and carries on. That is the right behaviour for the run and a dead end
 * for the operator: the pod logs stay empty while every `.hwp` fails, and the
 * only trace of it is inside a conversation nobody is reading.
 *
 ...
 * written down as well — which tool, which format, how long it took and whether
 * it answered — and "PDFs got slow last week" has evidence behind it.
 *
 ...
 *
 * **What must never appear here.** A document's content and the Markdown a
 * model wrote are the caller's, and neither is needed to diagnose anything — a
 * failing reader is identified by the tool and the format. A filename is
 * allowed on a failure line: it carries no capability, and it is what a person
 * reporting "my file would not open" will name. Nothing in this module accepts
 * more, which is the only reliable way to keep the rest out.
 */

/** The fields a caller may attach. Deliberately not `unknown` — see the module note. */
export interface LogContext {
  tool?: string;
  /** The format the call was about: the one requested of a writer, or a read file's extension. */
  format?: string;
  filename?: string;
 ...
 * every occurrence of one problem regardless of what the runtime called it that
 * day.
 */
